"""Optional AWQ external baseline for Milestone 4.

This runner compares a pre-quantized AWQ Transformers checkpoint against the
original model on the same text batch. It reports only shared end-to-end metrics
because AWQ uses its own packed/checkpoint runtime path rather than the
project's dequantized NumPy weight artifacts.
"""
...
@dataclass
class AwqBaselineConfig:
    model_name: str = "TinyLlama/TinyLlama-1.1B-Chat-v1.0"
    awq_model_name: str = "TinyLlama/TinyLlama-1.1B-Chat-v1.0"
    ...
    results_dir: Path = field(
        default_factory=lambda: Path("results/awq_tinyllama_baseline")
    )
...
def run_awq_baseline(config: AwqBaselineConfig) -> LogitRecord:
    if not config.evaluation_texts:
        raise ValueError("AwqBaselineConfig.evaluation_texts must not be empty")

    device = _resolve_torch_device(config.device)
    if device.type != "cuda":
        raise RuntimeError("AWQ baseline requires CUDA for this project run")
    ...
    awq_model.eval()
    awq_device = _first_parameter_device(awq_model) or device
    awq_inputs = [ids.to(awq_device) for ids in token_inputs]
    awq_logits, awq_loss = _forward_pass(awq_model, awq_inputs)
    ...
    _write_logit_csv(config.results_dir / "awq_logit_metrics.csv", [record])
    return record
...
def _make_logit_record(
    *,
    config: AwqBaselineConfig,
    original_logits: list[np.ndarray],
    original_loss: float,
    awq_logits: list[np.ndarray],
    awq_loss: float,
    ...
    awq_flat = np.concatenate([logits.flatten() for logits in awq_logits])
    logit_mse = float(np.mean((original_flat - awq_flat) ** 2))
    norm_original = np.linalg.norm(original_flat)
    norm_awq = np.linalg.norm(awq_flat)
    logit_cos = float(
        np.dot(original_flat, awq_flat) / (norm_original * norm_awq + 1e-8)
    )
    original_perplexity = math.exp(original_loss)
    awq_perplexity = math.exp(awq_loss)

    return LogitRecord(
        model_name=config.model_name,
        scope="external_baseline",
        method="external_awq_w4",
        bitwidth=4,
        ...
        top5_token_overlap=_top5_overlap(original_logits, awq_logits),
        loss=awq_loss,
        original_loss=original_loss,
        loss_delta=awq_loss - original_loss,
        perplexity=awq_perplexity,
        original_perplexity=original_perplexity,
        perplexity_ratio=awq_perplexity / original_perplexity,
...
def _collect_metadata(
    *,
    ...
    return {
        ...
        "model_name": config.model_name,
        "awq_model_name": config.awq_model_name,
...
def _write_metadata(results_dir: Path, metadata: dict[str, object]) -> Path:
    ...
    path = results_dir / "awq_metadata.json"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run an optional AWQ external baseline from a pre-quantized checkpoint.",
    )
    parser.add_argument(
        "--model-name",
    ...
    parser.add_argument(
        "--awq-model-name",
        required=True,
        help="Pre-quantized AWQ causal LM checkpoint to evaluate.",
    )
    parser.add_argument(
        "--eval-text-file",
        type=Path,
    ...
    parser.add_argument(
        "--results-dir",
        type=Path,
        default=Path("results/awq_tinyllama_baseline"),
        help="Directory for AWQ CSV and metadata.",
    )
    parser.add_argument(
        "--local-files-only",
        action="store_true",
    ...
    parser.add_argument(
        ...
        choices=["auto", "cuda"],
        default="auto",
        help="Device request; AWQ baseline is intended for CUDA.",
    )
    parser.add_argument(
        "--torch-dtype",
        choices=["float16", "bfloat16", "float32"],
        default="float16",
        help="dtype passed when loading the AWQ checkpoint.",
    )
    parser.add_argument(
        "--trust-remote-code",
        action="store_true",
        help="Allow remote model code if the selected AWQ checkpoint requires it.",
    )
    return parser


def build_config(args: argparse.Namespace) -> AwqBaselineConfig:
    ...
    return AwqBaselineConfig(
        model_name=args.model_name,
        awq_model_name=args.awq_model_name,
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    print(f"AWQ model: {config.awq_model_name}")
    ...
    if torch.cuda.is_available():
        torch.cuda.reset_peak_memory_stats()
    t0 = time.time()
    record = run_awq_baseline(config)
