"""Tests for the optional AWQ external baseline runner."""
...
from experiments import awq_baseline as awq


def _args(**overrides):
    defaults = dict(
        model_name="TinyLlama/TinyLlama-1.1B-Chat-v1.0",
        awq_model_name="test/tinyllama-awq",
        eval_text_file=None,
        max_eval_texts=None,
        results_dir=None,
...
def test_build_config_uses_eval_text_file(tmp_path):
    ...
    assert config.awq_model_name == "test/tinyllama-awq"


def test_resolve_torch_dtype_accepts_supported_values():
    assert awq._resolve_torch_dtype("float16") is torch.float16
    assert awq._resolve_torch_dtype("bfloat16") is torch.bfloat16
    assert awq._resolve_torch_dtype("float32") is torch.float32


def test_resolve_torch_dtype_rejects_unknown_value():
    with pytest.raises(ValueError, match="torch dtype"):
        awq._resolve_torch_dtype("float8")


def test_make_logit_record_computes_shared_metrics():
    config = awq.AwqBaselineConfig(
        model_name="test-model",
        awq_model_name="test-model-awq",
    ...
    assert record.method == "external_awq_w4"
...
def test_write_logit_csv_writes_expected_fields(tmp_path):
    ...
    record = awq._make_logit_record(
        ...
        original_loss=1.0,
        awq_logits=[np.array([[0.0, 1.1, 2.0, 2.9, 4.0]], dtype=np.float32)],
        awq_loss=1.1,
    )
    path = tmp_path / "metrics.csv"

    awq._write_logit_csv(path, [record])

    rows = list(csv.DictReader(path.open(newline="", encoding="utf-8")))
    assert len(rows) == 1
    assert rows[0]["method"] == "external_awq_w4"
...
def test_baseline_requires_cuda_before_loading_models(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="requires CUDA"):
        awq.run_awq_baseline(awq.AwqBaselineConfig())
